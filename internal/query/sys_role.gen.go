...
func newSysRole(db *gorm.DB, opts ...gen.DOOption) sysRole {
	...
	_sysRole.DataScope = field.NewInt16(tableName, "data_scope")
	_sysRole.ScopeDeptID = field.NewInt64(tableName, "scope_dept_id")
...
type sysRole struct {
	...
	RoleType    field.Int16  // 角色类型：1-系统预置(不可删除，核心权限不可剥离) 2-租户自定义
	DataScope   field.Int16  // 数据范围规则：1-仅本人 2-本部门 3-本部门及子部门 4-指定人 5-指定部门 6-全租户 7-用户组
	ScopeDeptID field.Int64  // 组织可见范围：限定角色可分配的部门层级。NULL=全租户可分配，具体部门ID=仅该部门及子部门可分配
...
func (s *sysRole) updateTableName(table string) *sysRole {
	...
	s.DataScope = field.NewInt16(table, "data_scope")
	s.ScopeDeptID = field.NewInt64(table, "scope_dept_id")
...
func (s *sysRole) fillFieldMap() {
	...
	s.fieldMap["scope_dept_id"] = s.ScopeDeptID
